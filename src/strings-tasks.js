...
function countVowels(str) {
  const vows = str.match(/[aeiouyAEIOUY]/g);
  return vows ? vows.length : 0;
}
...
function isPalindrome(str) {
  const r = str.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    res += index >= 0 ? output[index] : str[i];
  }
  return res;
}
